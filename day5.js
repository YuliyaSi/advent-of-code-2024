...
const countFirstMiddleSum = (strArr, ruleArr) => {
    let counter = 0

    strArr.forEach(strArr => {
    let valid = true;
    ruleArr.forEach(([rule1, rule2]) => {
        const ind1 = strArr.findIndex(el => el === rule1);
        const ind2 = strArr.findIndex(el => el === rule2);
        ...
        if (valid) counter += strArr[(strArr.length - 1)/2]
...
const testRules = [
    ...
    [75, 29], [61, 13], [53, 13], [75, 13],
    [75, 53], [29, 13], [97, 29], [53, 29],
    [61, 53], [97, 53], [61, 29], [47, 13],
    [75, 47], [97, 75], [47, 61], [75, 61], [47, 29],
]
...
const countMiddleSum = (arrOfStrs, arrOfRules) => {
    let counter = 0

    const countInvalid = (testedArray) => {
        let invalid = false;
        for (let i = 0; i < arrOfRules.length; i++) {
            const [rule1, rule2] = arrOfRules[i];
            const ind1 = testedArray.findIndex(el => el === rule1);
            const ind2 = testedArray.findIndex(el => el === rule2);
    ...
    const fixArray = (arrayToFix) => {
        let fixedArray;

        for (let i = 0; i < arrOfRules.length; i++) {
            const [rule1, rule2] = arrOfRules[i];
            const ind1 = arrayToFix.findIndex(el => el === rule1);
            const ind2 = arrayToFix.findIndex(el => el === rule2);
            if (ind1 >= 0 && ind2 >= 0 && ind1 > ind2) {
                arrayToFix[ind2] = rule1;
                arrayToFix[ind1] = rule2;
                fixArray(arrayToFix);
            } else {
    ...
    const filtered = arrOfStrs
        .filter(strArr => countInvalid(strArr))

    const fixedAres = filtered.map(badArray => fixArray(badArray));

    fixedAres.forEach(farr => counter += farr[(farr.length-1)/2])

    return counter
}

console.log(countFirstMiddleSum(strs, rules));
console.log(countMiddleSum(testStringArray, testRules));
console.log(countMiddleSum(strs, rules));
